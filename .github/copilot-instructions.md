...
- Parse the 'response' field from the result; strip any ```json fences (kept as defensive fallback even though format=json returns clean JSON)
- Raise HTTPException(502) on any failure
- All three calls (translate, get_synonyms, validate) share the module-level DECODE_OPTIONS constant
